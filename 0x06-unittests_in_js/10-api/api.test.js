...
describe('POST login', () => {
  it('if sucess', (done) => {
    const data = { userName: 'amgad' };
    request.post({url: 'http://localhost:7865/login', json: true, body: data},
      (error, response, body) => {
        expect(response.statusCode).to.equal(200);
  ...
  it('if fail', (done) => {
    request.post({url: 'http://localhost:7865/login', json: true, body: {}},
      (error, response, body) => {
...
describe('available_payments', () => {
  it('test result', (done) => {
    request.get(`http://localhost:7865/available_payments`, {json: true}, (error, response, body) => {
      expect(response.statusCode).to.equal(200);
